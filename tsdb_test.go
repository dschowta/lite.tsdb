...
func TestTsdb_Add(t *testing.T) {
	...
	query := Query{Series: tname, MaxEntries: 100, From: series[0].Time, To: series[len(series)-1].Time, Sort: ASC}
...
func TestTsdb_CheckDescending(t *testing.T) {
	...
	query := Query{Series: tname, MaxEntries: 100, From: series[0].Time, To: series[len(series)-1].Time, Sort: DESC}
	resSeries, nextEntry, err := db.Query(query)
	if err != nil {
		t.Fatal(err)
	}
...
func TestTsdb_QueryPagination(t *testing.T) {
	...
	query := Query{Series: tname, MaxEntries: 50, From: series[0].Time, To: series[len(series)-1].Time, Sort: ASC}
	resSeries, nextEntry, err := db.Query(query)
	if err != nil {
		t.Fatal(err)
	}
	...
	query = Query{Series: tname, MaxEntries: 50, From: *nextEntry, To: series[len(series)-1].Time, Sort: ASC}
	resSeries, nextEntry, err = db.Query(query)
	if err != nil {
		t.Fatal(err)
	}
...
func TestTsdb_QueryPages(t *testing.T) {
	...
	query := Query{Series: tname, MaxEntries: limit, From: series[0].Time, To: series[len(series)-1].Time, Sort: ASC}
